...
All notable changes to `/vidsense` are documented here.

## [0.1.0] — Initial release

### Added
- `/vidsense <url-or-path> [question]` slash command.
...
- `~/.config/vidsense/.env` only — no cwd `.env` fallback. Avoids silently picking up keys from random project directories.
...
- GitHub Actions workflow auto-builds `dist/vidsense.skill` on tag push and attaches to the release.
