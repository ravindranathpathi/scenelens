...
homepage: https://github.com/YOUR_GITHUB_HANDLE/vidsense
repository: https://github.com/YOUR_GITHUB_HANDLE/vidsense
author: YOUR_NAME
license: MIT
user-invocable: true
---

# /vidsense — Claude watches a video, smarter

You don't have a video input; this skill gives you one. Compared to a fixed-fps frame grab, vidsense:
...
Before every `/vidsense` call, verify dependencies and an API key are in place:
...
**If an API key is still missing after install:** use `AskUserQuestion` to ask whether the user has a Groq API key (preferred — cheaper, faster) or an OpenAI key, then write it into `~/.config/vidsense/.env` on the matching `GROQ_API_KEY=...` or `OPENAI_API_KEY=...` line. If they don't want Whisper, proceed with `--no-whisper` and tell them captions-less videos come back frames-only.
...
- User types `/vidsense <url-or-path> [question]`.

## How to invoke

**Step 1 — parse user input.** Separate the video source from any question. `/vidsense https://youtu.be/abc what hook did they open with?` → source = `https://youtu.be/abc`, question = `what hook did they open with?`.

**Step 2 — run the script.** Pass the source verbatim:

```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/vidsense.py" "<source>"
```
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/vidsense.py" video.mp4 --start 50 --end 60
python3 "${CLAUDE_SKILL_DIR}/scripts/vidsense.py" "$URL" --start 2:15 --end 2:45
python3 "${CLAUDE_SKILL_DIR}/scripts/vidsense.py" "$URL" --start 1:12:00
```
...
Both keys live in `~/.config/vidsense/.env`. Unlike skills that fall back to a project-local `.env`, vidsense reads ONLY from `~/.config/vidsense/.env` and process env — to avoid silently picking up keys from random project directories.

## Failure modes

- **Setup preflight failed** → run `python3 "${CLAUDE_SKILL_DIR}/scripts/setup.py"` (auto-installs ffmpeg/yt-dlp via brew on macOS, scaffolds `.env`). For an API key, ask the user via `AskUserQuestion` and write it to `~/.config/vidsense/.env`.
...
- Reads / creates `~/.config/vidsense/.env` (mode `0600` on POSIX) for the Whisper API key(s) and a `SETUP_COMPLETE` marker.
...
**Bundled scripts:** `scripts/vidsense.py` (entry), `scripts/download.py` (yt-dlp wrapper), `scripts/frames.py` (scene + fixed-fps extraction), `scripts/ocr.py` (Tesseract wrapper), `scripts/transcribe.py` (VTT parser), `scripts/whisper.py` (Groq/OpenAI clients with chunking), `scripts/setup.py` (preflight + installer).
